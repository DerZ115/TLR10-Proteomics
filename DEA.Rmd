...
data <- read_DIA_report("data/withMS1/20240626_081714_ADIPO_ASC_WCL_TLR10_Light_Report.tsv")
...
data.pca <- assay(data.imputed, "MS2") %>%
  t() %>%
  prcomp() %$% x %>%
  as.data.frame() %>%
  rownames_to_column("Sample.Name") %>%
...
data.merged <- merge(as.data.frame(assay(data.imputed, "MS1")),
                     as.data.frame(assay(data.imputed, "MS2")),
...
cont_matrix <- makeContrasts(LightVsDarkInWT = ADIPO_ASC_WT.Light - ADIPO_ASC_WT.Dark,
...
fit3$count <- rowData(data.imputed)[rownames(fit3$coefficients),]$Peptides
...
if (is.na(vp_max_labels) || nrow(res.ASC_WT.sig) < vp_max_labels) {
  ...
  res.ASC_WT.plot %<>% arrange(desc(threshold), sca.adj.pval) %>%
