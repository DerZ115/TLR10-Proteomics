...
quant_matrix.log.norm <- quant_matrix %>%
...
missing_values <- quant_data.log.norm %>% 
  mutate(na_vals = rowSums(is.na(.)))
...
quant_matrix.impute <- MsCoreUtils::impute_mixed(quant_matrix.log.norm, !MNAR, "MLE", "min")
...
samples <- samples %>% mutate(class = factor(paste(Cell.Type, Condition, sep = "."),
                                   levels = c("ASC_WT.Dark", "ASC_WT.Light",
                                              "ASC_TLR10_LOV.Dark", "ASC_TLR10_LOV.Light")))

DE_design <- model.matrix(~ 0 + samples$class)
colnames(DE_design) <- levels(samples$class)

fit1 <- lmFit(quant_matrix.impute, design = DE_design)
...
current_contrast <- 1
DEqMS_res <- outputResult(fit4, coef_col = 1)
DEqMS_res_q <- DEqMS_res %>% mutate(qval = qvalue(P.Value)$qvalue)
